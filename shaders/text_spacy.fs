/*{
  ...
  "DESCRIPTION": "Spacy - perspective tunnel rows, lava lamp oil diffusion background",
  "INPUTS": [
    ...
    { "NAME": "textColor", "LABEL": "Color", "TYPE": "color", "DEFAULT": [1.5, 1.2, 0.8, 1.0] },
    { "NAME": "bgColor", "LABEL": "Background", "TYPE": "color", "DEFAULT": [0.18, 0.01, 0.0, 1.0] },
...
// =======================================================================
// BACKGROUND: Lava Lamp — amber/orange metablobs rising in deep crimson
// =======================================================================
vec3 lavaLampBg(vec2 uv) {
    float ap = (audioBass.x+audioBass.y+audioBass.z+audioBass.w)*0.25;
    // Deep crimson fluid base
    vec3 fluid = bgColor.rgb;
    // Metaball-style blobs: soft sum of Gaussian bumps
    float heat = 0.0;
    for (int i = 0; i < 6; i++) {
        float fi = float(i);
        float phase = TIME*0.18*(0.7+fi*0.15) + fi*2.1;
        float bx = 0.2 + hash(fi*5.3)*0.6;
        float by = fract(phase * 0.3);           // loops 0→1
        float bsize = 0.08 + hash(fi*11.7)*0.07 + ap*0.02;
        // Wobble
        float wx = sin(phase*1.3+fi)*0.04;
        float wy = sin(phase*0.9)*0.02;
        vec2 bc = vec2(bx+wx, by+wy);
        float d2 = dot(uv-bc,uv-bc);
        heat += bsize*bsize / (d2 + 0.001);
    }
    // Map heat to amber → orange → gold → white-hot
    float h = smoothstep(0.0, 3.0, heat);
    vec3 ambCol = h < 0.33 ? mix(vec3(0.6,0.08,0.0), vec3(2.0,0.7,0.0), h/0.33) :
                  h < 0.66 ? mix(vec3(2.0,0.7,0.0),   vec3(3.0,1.8,0.3), (h-0.33)/0.33) :
                              mix(vec3(3.0,1.8,0.3),   vec3(3.5,3.0,2.0), (h-0.66)/0.34);
    // fwidth AA on blob boundary
    float fw = fwidth(heat);
    float blobEdge = smoothstep(fw*2.0, 0.0, abs(heat - 1.5) - fw);
    vec3 col = mix(fluid, ambCol, smoothstep(0.5, 2.5, heat));
    col *= 1.0 - blobEdge * 0.5; // dark ring at blob boundary
    return col;
}

// =======================================================================
// EFFECT: SPACY with lava lamp background
// =======================================================================
...
void main() {
    ...
    if (_voiceGlitch > 0.01) {
        ...
        vec4 cR = effectSpacy(uvR, p); vec4 cG = effectSpacy(uvG, p); vec4 cB = effectSpacy(uvB, p);
