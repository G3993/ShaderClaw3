/*{
  "DESCRIPTION": "Data Sculpture + Soap Bubble — five abstract data-art moods (Bar Forest, Particle Stream, Skyline Grid, Fingerprint Field, Ribbon Cloud) with an optional iridescent soap-bubble overlay. Synthetic 32-band FFT, hash-driven and audio-modulated. Single-pass LINEAR HDR.",
  ...
  "INPUTS": [
    ...
    { "NAME": "bubbleSize",  "LABEL": "Bubble Size",    "TYPE": "float", "MIN": 0.0, "MAX": 1.5,  "DEFAULT": 0.72 },
    { "NAME": "bubbleMix",   "LABEL": "Bubble Mix",     "TYPE": "float", "MIN": 0.0, "MAX": 1.0,  "DEFAULT": 0.55 },
...
// ════════════════════════════════════════════════════════════════════════
//  Data Sculpture × Soap Bubble fusion
//  Five data-art moods + iridescent raymarched bubble overlay.
// ════════════════════════════════════════════════════════════════════════
...
// ─── Soap Bubble SDF / Raymarch ──────────────────────────────────────
float sdSphere(vec3 pp, float r) { return length(pp) - r; }

vec3 sphNorm(vec3 pp, float r) {
    vec2 e = vec2(EPS, 0.0);
    return normalize(vec3(
        sdSphere(pp + e.xyy, r) - sdSphere(pp - e.xyy, r),
        sdSphere(pp + e.yxy, r) - sdSphere(pp - e.yxy, r),
        sdSphere(pp + e.yyx, r) - sdSphere(pp - e.yyx, r)
    ));
}

// Returns iridescent bubble colour + hit mask.
// uvScreen : normalised [0,1] frag coord (isf_FragNormCoord)
// bass/treble: audio components
vec4 bubbleLayer(vec2 uvScreen, float t, float bass, float treble) {
    // Build camera ray
    vec2 sc = uvScreen * 2.0 - 1.0;
    sc.x *= RENDERSIZE.x / RENDERSIZE.y;

    float ang = t * orbitSpeed;
    vec3 ro  = vec3(sin(ang) * 2.8, 0.35 + 0.2 * sin(t * 0.22), cos(ang) * 2.8);
    vec3 fwd = normalize(-ro);
    vec3 rgt = normalize(cross(vec3(0.0, 1.0, 0.0), fwd));
    vec3 upV = cross(fwd, rgt);
    vec3 rd  = normalize(fwd + rgt * sc.x * 0.65 + upV * sc.y * 0.65);
    ...
    for (int i = 0; i < MAX_STEPS; i++) {
        float dd = sdSphere(ro + rd * tm, R);
        if (dd < EPS) { hit = true; break; }
        if (tm > MAX_DIST) break;
        tm += dd;
    }
    ...
    float hue     = fract(fres * filmTurns + posAng * 0.5
                          + t * filmSpeed * 0.04 + treble * 0.15);
    vec3 film1    = hsv2rgb(vec3(hue, 1.0, 1.0)) * 1.6;
    float hue2    = fract(hue + 0.37 + t * filmSpeed * 0.017);
    ...
    vec3 bCol  = film * (0.22 + 0.78 * dKey + dFill);
    bCol += vec3(1.0,  0.97, 0.93) * spK * specPeak;
    bCol += vec3(0.65, 0.88, 1.0)  * spF * specPeak * 0.4;

    // Fresnel rim — translucent silhouette
    float rim = pow(fres, 3.5);
    bCol = mix(bCol, vec3(0.0), rim * 0.32);

    // Audio breath
    bCol += film * bass * 0.35;

    // Alpha: opaque inside, fade at rim for glass feel
    float alpha = clamp(1.0 - rim * 0.85, 0.15, 1.0);

    return vec4(bCol, alpha);
}
...
void main() {
    ...
    // Soap bubble overlay (only if bubbleSize > 0 and bubbleMix > 0)
    if (bubbleSize > 0.01 && bubbleMix > 0.001) {
        vec4 bubble = bubbleLayer(uv, t, bass, treble);
        // Additive-ish blend: preserve data art, add iridescent glass on top
        float alpha = bubble.a * bubbleMix;
        col = mix(col, col * (1.0 - alpha * 0.6) + bubble.rgb * alpha, 1.0);
    }
