/*{
  ...
  "DESCRIPTION": "Bricks - grid with animated displacement, sakura night background",
  "INPUTS": [
    ...
    { "NAME": "textColor", "LABEL": "Color", "TYPE": "color", "DEFAULT": [1.0, 0.85, 0.2, 1.0] },
    { "NAME": "bgColor", "LABEL": "Background", "TYPE": "color", "DEFAULT": [0.02, 0.01, 0.12, 1.0] },
...
float hash2s(vec2 p){ return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5); }

// =======================================================================
// BACKGROUND: Sakura Night Sky — deep indigo, falling pink petals
// =======================================================================
vec3 sakuraBg(vec2 uv) {
    vec3 bg = bgColor.rgb;
    float ap = (audioBass.x+audioBass.y)*0.5;
    for (int i = 0; i < 6; i++) {
        float layer = float(i)*7.3 + TIME*0.18;
        vec2 p = uv * mix(3.0, 9.0, float(i)/6.0);
        p.y += fract(layer)*2.5;
        p.x += sin(p.y*0.5+layer*0.3)*0.25*(1.0+ap*0.2);
        vec2 cell = floor(p);
        vec2 lp = fract(p) - 0.5;
        float seed = hash2s(cell + float(i)*41.7);
        if (seed > 0.32) {
            float pd = length(lp) - 0.1 - seed*0.04;
            float petal = smoothstep(0.012, 0.0, pd);
            float bright = (1.0 - float(i)/6.0) * 2.0 * (1.0+ap*0.3);
            bg += vec3(2.5, 0.3, 0.9) * petal * bright * seed;
        }
    }
    bg += vec3(0.07, 0.0, 0.18) * smoothstep(0.9, 0.1, uv.y);
    return bg;
}

// =======================================================================
// EFFECT: BRICKS with sakura background
// =======================================================================
vec4 effectBricks(vec2 uv, int sub) {
    ...
    if (transparentBg) { a = textHit; fc = textColor.rgb; }
        ...
        vec3 bg = sakuraBg(uv);
        float ap = (audioBass.x+audioBass.y)*0.5;
        fc = bg*(1.0-textHit*0.6) + textColor.rgb*hdrGlow*(1.0+ap*0.25)*textHit;
    }
    return vec4(fc, a);
}
...
void main() {
    ...
    if (_voiceGlitch > 0.01) {
        ...
        vec4 cR = effectBricks(uvR, p); vec4 cG = effectBricks(uvG, p); vec4 cB = effectBricks(uvB, p);
