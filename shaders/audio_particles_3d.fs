/*{
  ...
  "DESCRIPTION": "Audio Particles 3D — particle-as-subject in five curated moods. Anadol Cloud, Gursky Repetition, Memo Form, Constellation, and Turrell Glow (volumetric single-hue luminous field after James Turrell's Roden Crater). Orbit K ≤ 1.5 per motion discipline. LINEAR HDR out.",
  "INPUTS": [
    { "NAME": "mood",          "LABEL": "Mood",            "TYPE": "long",  "DEFAULT": 0, "VALUES": [0,1,2,3,4], "LABELS": ["Anadol Cloud","Gursky Repetition","Memo Form","Constellation","Turrell Glow"] },
...
void main() {
    ...
    // Universal camera: orbit K capped to 1.5 per motion discipline §2.
    // Was audio.y * 0.6 additive (K=3.3 relative to default); now multiplicative K=1.5.
    float orbT = camAzimuth + TIME * (camOrbitSpeed * (1.0 + audio.y * 1.5));
    ...
    else if (m == 4) { bgT = vec3(0.002, 0.001, 0.006); bgB = vec3(0.001, 0.001, 0.002); }
    ...
    if      (m == 0) emit = evalCloud(ro, rd, TIME, audio, cloudCount(tier),
                                      ...
                                      kDir, keyColor.rgb, fillColor.rgb, ambient, rimStrength);
    else if (m == 3) emit = evalConstellation(ro, rd, TIME, audio, starCount(tier));
    else {
        // Mood 4: Turrell Glow — volumetric single-hue luminous field.
        // After James Turrell's Roden Crater perceptual light installations.
        // A pure emissive fog whose hue slowly cycles through the spectrum
        // (deep violet → pure magenta → electric blue → cobalt → back).
        // Bass aggregates density; treble brightens the hue peak.
        // No particle positions — it's a volumetric integral along the ray.
        float hueBase = TIME * 0.02 + audio.x * 0.08;  // slow hue drift
        float h6 = fract(hueBase) * 6.0;
        vec3 hueCol;
        hueCol.r = clamp(abs(h6 - 3.0) - 1.0, 0.0, 1.0);
        hueCol.g = clamp(2.0 - abs(h6 - 2.0), 0.0, 1.0);
        hueCol.b = clamp(2.0 - abs(h6 - 4.0), 0.0, 1.0);
        // Saturate fully: eliminate any grey residue
        float maxC = max(hueCol.r, max(hueCol.g, hueCol.b));
        if (maxC > 0.0) hueCol /= maxC;

        // HDR: field luminance 1.8-2.5 linear so bloom picks it up hard.
        float density = (0.65 + audio.x * 1.2) * (1.0 + audio.z * 0.5);
        // Ray-marched fog integral: accumulate along ray in screen space.
        // Use the angular distance from the field's "horizon" to add depth.
        float horizon = clamp(1.0 - abs(rd.y) * 1.5, 0.0, 1.0);
        float fogVol  = horizon * density;
        // Inner glow brightens near the camera center (Turrell's aperture)
        float aperture = exp(-dot(uv, uv) * 0.6);
        float peakLum  = 1.80 + audio.z * 0.60;   // K_treble = 0.6/1.8 = 0.33 ≤ 1.5 ✓
        emit = hueCol * fogVol * peakLum * (0.5 + aperture * 0.8);
    }
    ...
    if (m == 4) col  = max(col, bg);                       // protect Turrell void
