/*{
  ...
  "INPUTS": [
    ...
    { "NAME": "skyTopColor",      "LABEL": "Sky Top",         "TYPE": "color", "DEFAULT": [0.04, 0.02, 0.30, 1.0] },
    { "NAME": "skyHorizonColor",  "LABEL": "Sky Horizon",     "TYPE": "color", "DEFAULT": [0.20, 0.0,  0.45, 1.0] },
    ...
    { "NAME": "holoTint",         "LABEL": "Hologram Tint",   "TYPE": "color", "DEFAULT": [0.40, 0.80, 1.0,  1.0] },
...
vec4 passVapor(vec2 fragCoord) {
    ...
    if (length(sd) < sr) {
        ...
        vec3 sunC = mix(vec3(0.0, 0.80, 1.0), vec3(0.30, 0.0, 0.90), ty); // cyan→violet sun
        if (sunBars > 0.0) {
            float barY = sd.y / sr;
            float barMask = step(0.0, sin(barY * sunBars * 3.14159 + 0.4 + TIME * 0.5));
            sunC = mix(sunC, sky, barMask * 0.55);
        }
        col = sunC * 2.2; // HDR cyan/violet sun
    }
    ...
    if (uv.y < horizonY) {
        ...
        vec3 floorBase = mix(vec3(0.02, 0.04, 0.20),
                             vec3(0.08, 0.0,  0.35), uv.y / horizonY);
        col = mix(floorBase, vec3(0.0, 0.70, 1.0) * 2.0, line); // electric blue grid HDR
    ...
    for (int i = 0; i < 20; i++) {
        ...
        float hue  = fract(h2 * 0.45 + 0.50 + TIME * 0.05); // bias toward cool hues (cyan/violet range)
        vec3 shapeCol = hsv2rgb(vec3(hue, 1.0, 1.0)); // fully saturated
        ...
        if (dist < 0.0) col = mix(col, shapeCol * 2.2, vis); // HDR 2.2× shapes
        col = mix(col, vec3(0.6, 0.9, 1.0) * 3.0, smoothstep(0.04, 0.0, abs(dist)) * vis * 0.5); // cyan outline
    }
    ...
    // Katakana ribbon (top)
    {
        ...
        col = mix(col, vec3(0.4, 0.9, 1.0) * 2.5, total * katakanaIntensity); // ice-blue katakana HDR
    }
...
vec4 passHologram(vec2 fragCoord) {
    ...
    // Transmission strength — fixed: never drops below 85% without audio
    holo *= max(0.85, 0.7 + audioLevel * audioReact * 0.4);
