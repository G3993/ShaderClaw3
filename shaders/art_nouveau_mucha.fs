/*{
  ...
  "DESCRIPTION": "Art Nouveau (Mucha) — Garden Portrait. 3D raymarched bust in profile now set in an outdoor garden: encircling vine SDFs (chained capsule tendrils with ellipsoid leaf buds), SDF flower cartouche replaces star halo. Lighting: warm golden sun key from upper-right, cool sky fill from zenith, dappled ambient. NEW PALETTE: deep violet/verdant green/ivory instead of studio gold/rose. Audio pulses vine radius; treble shimmers leaf buds. Returns LINEAR HDR.",
  "INPUTS": [
    ...
    { "NAME": "keyColor",      "LABEL": "Key Color",       "TYPE": "color", "DEFAULT": [1.60, 1.35, 0.80, 1.0] },
    { "NAME": "fillColor",     "LABEL": "Fill Color",      "TYPE": "color", "DEFAULT": [0.45, 0.68, 0.92, 1.0] },
...
// ─── palette ────────────────────────────────────────────────────────────
// Garden portrait palette: deep violet, verdant green, ivory, sun gold.
const vec3 GOLD   = vec3(2.30, 1.60, 0.40);   // sunlight accent HDR
const vec3 CREAM  = vec3(0.94, 0.90, 0.80);   // ivory bust
const vec3 ROSE   = vec3(0.45, 0.08, 0.55);   // deep violet flower
const vec3 TEAL   = vec3(0.12, 0.45, 0.20);   // verdant leaf green
const vec3 BONE   = vec3(0.96, 0.93, 0.82);   // light ivory

vec3 moodTint(int m){
    if (m == 0) return TEAL  * 0.55 + ROSE * 0.12;       // Reverie — violet-green
    if (m == 1) return ROSE  * 0.70 + GOLD * 0.08;       // Médée — violet+gold
    if (m == 2) return CREAM * 0.65 + TEAL * 0.10;       // Camélias — ivory+green
    return GOLD * 0.28 + TEAL * 0.12;                    // Job — sun+leaf
}
...
// ─── Garden vine tendrils ────────────────────────────────────────────────
// Chained capsule segments that spiral around the bust like Art Nouveau
// vine borders. Leaf buds are tiny ellipsoids attached at knuckle points.
// id 6 = vine stem (green), id 7 = leaf/flower bud (violet)
void sdVines(vec3 p, float t, float audio, out float stemD, out float leafD) {
    stemD = 1e9; leafD = 1e9;
    // 3 vine arms spiraling at different rates
    for (int arm = 0; arm < 3; arm++) {
        float fa = float(arm);
        float baseAngle = fa * 2.094 + t * 0.25;
        // 4 segments per arm, each marching further up
        vec3 prev = vec3(cos(baseAngle)*0.70, -0.65, sin(baseAngle)*0.50);
        for (int seg = 0; seg < 4; seg++) {
            float fs = float(seg);
            float a = baseAngle + fs * 0.62 + t * 0.12;
            float yUp = -0.55 + fs * 0.35;
            float r = 0.65 - fs * 0.06;
            // Vine pulse: audio bass modulates tightness (K ≤ 1.5 ✓)
            r *= 1.0 + audio * 0.18;   // K=0.18 per segment ≤ 1.5 ✓
            vec3 next = vec3(cos(a)*r, yUp, sin(a)*r*0.65);
            // Stem capsule
            vec3 pa = p - prev, ba = next - prev;
            float h = clamp(dot(pa,ba)/dot(ba,ba), 0.0, 1.0);
            float ds = length(pa - ba*h) - 0.020;
            stemD = min(stemD, ds);
            // Leaf bud at joint
            float dLeaf = length(p - mix(prev, next, 0.5)) / 0.022 - 1.0;
            // Ellipsoid approximation: flatten along vine direction
            vec3 bn = normalize(ba + vec3(0.0001));
            float along = dot(p - mix(prev, next, 0.5), bn);
            dLeaf = max(dLeaf, abs(along) - 0.038);
            leafD = min(leafD, dLeaf);
            prev = next;
        }
    }
}
...
Hit map(vec3 p, float t, int mood, int nR, float rR, float twist,
    ...
    // Garden vines — encircle the bust with green stems (id 6) and violet
    // leaf buds (id 7). The vine geometry is always present in garden portrait.
    float vStem, vLeaf;
    sdVines(p, t, audio, vStem, vLeaf);
    h = hitMin(h, Hit(vStem, 6));
    h = hitMin(h, Hit(vLeaf, 7));
...
vec3 shade(vec3 pos, vec3 nor, vec3 rd, int id, vec3 keyDir, vec3 keyCol,
    ...
    else if (id == 6){ base = TEAL * 0.80;                          spec = 0.08; gloss = 12.0; }  // vine stem
    else if (id == 7){ base = ROSE * 1.10;                          spec = 0.18; gloss = 32.0; }  // violet bud
...
void main() {
    ...
    // ─── Background: garden sky — pale sky blue top, warm green horizon ──
    vec3 skyTop = vec3(0.55, 0.72, 0.92);   // open blue sky
    vec3 skyBot = vec3(0.38, 0.58, 0.28);   // green garden foliage horizon
    vec3 bg = mix(skyBot, skyTop, clamp(ndc.y * 0.8 + 0.5, 0.0, 1.0));
    bg += moodTint(mt) * 0.12;
    bg *= 0.88 + 0.12 * length(ndc);
