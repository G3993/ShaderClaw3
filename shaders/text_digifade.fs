/*{
  ...
  "DESCRIPTION": "Digifade - glitch dissolve, UV blacklight fluorescent poster background",
  "INPUTS": [
    ...
    { "NAME": "bgColor", "LABEL": "Background", "TYPE": "color", "DEFAULT": [0.04, 0.0, 0.12, 1.0] },
    { "NAME": "transparentBg", "LABEL": "Transparent", "TYPE": "bool", "DEFAULT": false },
    { "NAME": "hdrGlow", "LABEL": "HDR Glow", "TYPE": "float", "MIN": 0.5, "MAX": 5.0, "DEFAULT": 2.8 },
...
float hash2uv(vec2 p){ return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5); }

// =======================================================================
// BACKGROUND: UV Blacklight — deep violet, fluorescent splatter spots
// =======================================================================
vec3 uvBlacklightBg(vec2 uv) {
    vec3 bg = bgColor.rgb;
    float ap = (audioBass.x+audioBass.y+audioBass.z+audioBass.w)*0.25;
    // Fluorescent colors: hot pink, electric cyan, acid lime
    vec3 fluors[3];
    fluors[0] = vec3(2.8, 0.0, 2.2); // hot pink
    fluors[1] = vec3(0.0, 2.5, 2.5); // electric cyan
    fluors[2] = vec3(0.4, 3.0, 0.0); // acid lime
    // Scattered splatter dots
    for (int i = 0; i < 18; i++) {
        float fi = float(i);
        vec2 center = vec2(hash(fi*3.1), hash(fi*7.7));
        float r = 0.03 + hash(fi*13.3)*0.06;
        float d = length(uv - center) - r;
        float blob = smoothstep(fwidth(d)*2.0, 0.0, d);
        int ci = int(mod(fi, 3.0));
        vec3 fc;
        if(ci==0) fc=fluors[0]; else if(ci==1) fc=fluors[1]; else fc=fluors[2];
        float pulse = 0.8 + 0.2*sin(TIME*1.5+fi*2.3) + ap*0.3;
        bg += fc * blob * pulse;
    }
    // Faint UV grid lines (blacklight poster grid)
    vec2 grid = fract(uv * 8.0 + 0.5);
    float gLine = smoothstep(fwidth(grid.x)*3.0,0.0,abs(grid.x-0.5)-0.46);
    gLine = max(gLine, smoothstep(fwidth(grid.y)*3.0,0.0,abs(grid.y-0.5)-0.46));
    bg += vec3(0.5, 0.0, 1.2) * gLine * 0.08;
    // Slight violet vignette inward
    bg += vec3(0.1, 0.0, 0.3) * (1.0 - length(uv-0.5)*1.4);
    return bg;
}

// =======================================================================
// EFFECT: DIGIFADE with UV background
// =======================================================================
vec4 effectDigifade(vec2 uv, int sub) {
    ...
    if (transparentBg) { a = textHit; fc = textColor.rgb; }
        ...
        vec3 bg = uvBlacklightBg(uv);
        float ap = (audioBass.x+audioBass.y+audioBass.z+audioBass.w)*0.25;
        fc = bg*(1.0-textHit*0.6) + textColor.rgb*hdrGlow*(1.0+ap*0.3)*textHit;
    }
    return vec4(fc, a);
}
...
void main() {
    ...
    if (_voiceGlitch > 0.01) {
        ...
        vec4 cR = effectDigifade(uvR, p); vec4 cG = effectDigifade(uvG, p); vec4 cB = effectDigifade(uvB, p);
