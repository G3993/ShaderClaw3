/*{
  ...
  "DESCRIPTION": "Morphing metallic blobs via ray-marched metaballs — texture revealed through the shapes",
  "INPUTS": [
...
float scene(vec3 p, float t, float cnt) {
  float d = 20.0;
...
vec3 calcNormal(vec3 p, float t, float cnt) {
...
void main() {
  ...
  float t = TIME;
  float cnt = blobCount + audioBass * 2.0;

  vec3 ro = vec3(0.0, 0.3, 3.8);
  vec3 target = vec3(0.0);
  if (mousePos.x > 0.01 || mousePos.y > 0.01) {
    target = vec3((mousePos - 0.5) * 1.5, 0.0) * 0.3;
  }
  ...
  if (hit > 0.5) {
    ...
    if (texSample.a > 0.01) {
      ...
      vec2 refractUV = screenUV + n.xy * 0.05;
      ...
      vec3 albedo = mix(vec3(0.85, 0.65, 0.3), vec3(0.75, 0.45, 0.25),
        sin(p.x * 3.0 + p.z * 2.0 + t * morphSpeed * 0.4) * 0.5 + 0.5);
